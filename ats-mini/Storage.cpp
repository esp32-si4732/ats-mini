...
void prefsSave(uint32_t items)
{
  if(items & SAVE_SETTINGS)
  {
    ...
    prefs.putUShort("SleepTmr",   currentSleepTimer); // Sleep timer
    prefs.putBool("SleepSmart",   sleepSmart);      // Smart sleep mode
    prefs.putUChar("SleepDaySt",  sleepDayStart);   // Smart sleep day start hour
    prefs.putUChar("SleepNightSt",sleepNightStart); // Smart sleep night start hour
...
bool prefsLoad(uint32_t items)
{
  if(items & SAVE_SETTINGS)
  {
    ...
    currentSleepTimer = prefs.getUShort("SleepTmr", 0);        // Sleep timer
    sleepSmart      = prefs.getBool("SleepSmart", sleepSmart);
    sleepDayStart   = prefs.getUChar("SleepDaySt", sleepDayStart);
    sleepNightStart = prefs.getUChar("SleepNightSt", sleepNightStart);
